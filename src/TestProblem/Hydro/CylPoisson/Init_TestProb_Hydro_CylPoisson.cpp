...
static double Sphere_Radius;        // sphere radius
static double Sphere_Center[3];     // sphere center - in cylindrical 
// =======================================================================================


static void Output_L1Error_CylPoisson();
...
// ********************************************************************************************************************************
   ReadPara->Add( "Sphere_Dens",       &Sphere_Dens,           -1.0,          Eps_double,       NoMax_double      );
...
// set the default explosion center
   for (int d=0; d<3; d++)
      if ( Sphere_Center[d] == NoDef_double )    Sphere_Center[d] = amr->BoxCenter[d];
...
// (4) make a note
   if ( MPI_Rank == 0 )
   {
      ...
      Aux_Message( stdout, "  sphere cartesian center   = (%13.7e, %13.7e, %13.7e)\n", Sphere_Center[0], Sphere_Center[1],
                                                                                       Sphere_Center[2] );
      Aux_Message( stdout, "=============================================================================\n" );
   }
                ...
                const int lv, double AuxArray[] )
{
   ...
   const double background_dens = Sphere_Dens * 1e-8;
   const double background_engy = 0.0;
   
   const double x1 = x * COS(y) ;
   const double x2 = x * SIN(y) ;
   const double x3 = z ;
   const double Sphere_C1 = Sphere_Center[0] * COS(Sphere_Center[1]);
   const double Sphere_C2 = Sphere_Center[0] * SIN(Sphere_Center[1]);
   const double Sphere_C3 = Sphere_Center[2];
   
   const double r = SQRT( SQR(x1-Sphere_C1) + SQR(x2-Sphere_C2) + SQR(x3-Sphere_C3) );
   ...
   else                       fluid[DENS] = background_dens;
...
void Output_L1Error_CylPoisson(){
   ...
   const double sphere_x1 = Sphere_Center[0] * COS(Sphere_Center[1]);
   const double sphere_x2 = Sphere_Center[0] * SIN(Sphere_Center[1]);
   const double sphere_x3 = Sphere_Center[2];
      ...
      for (int PID=0; PID<amr->NPatchComma[lv][1]; PID++) {
         if ( amr->patch[0][lv][PID]->son == -1 ) {
            ...
            for (int i=0; i<PATCH_SIZE; i++) 
            {
               ...
               dist2center = SQRT( SQR(x_crt-sphere_x1) + SQR(y_crt-sphere_x2) + SQR(z_crt-sphere_x3) );
